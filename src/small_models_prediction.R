...
load(file = "output/esm_em_out.RData")
load(file = "output/esm_out.RData")
...
for(i in species_list_unbalance){

  # take the variable names accordingly

  var <- var_unbalannce_list[[i]]
  # Predicting single model
  my.ESM_proj_current<-ecospat.ESM.Projection(ESM.modeling.output=my.ESM,
                                              new.env=preds[[var]])
  #  Predicting of calibrated ESMs
  current.projections[[i]] <- ecospat.ESM.EnsembleProjection(ESM.prediction.output=my.ESM_proj_current,
